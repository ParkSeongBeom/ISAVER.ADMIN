...
<c:set value="MN000000-D000-0000-0000-000000000004" var="menuId"/>
...
<section class="container sub_area">
    ...
    <article class="sub_title_area">
        <!-- 2depth 타이틀 Start-->
        <h3 class="1depth_title"><spring:message code="common.title.logauthadminuser"/></h3>
        <!-- 2depth 타이틀 End -->
    ...
    <form id="logAuthAdminUserForm" method="POST">
        ...
        <article class="search_area">
            <div class="search_contents">
                <!-- 일반 input 폼 공통 -->
                <p class="itype_01">
                    <span><spring:message code="logauthadminuser.column.adminId" /></span>
                    <span>
                        <input type="text" name="id" value="${paramBean.id}"/>
                    </span>
                </p>
                <p class="itype_01">
                    <span><spring:message code="logauthadminuser.column.status"/></span>
                    <span>
    ...
    <article class="table_area">
        <div class="table_contents">
            ...
            <table class="t_defalut t_type01 t_style02 cursor_no">
                ...
                <thead>
                    <tr>
                        <th><spring:message code="logauthadminuser.column.logType"/></th>
                        <th><spring:message code="logauthadminuser.column.status"/></th>
                        <th><spring:message code="logauthadminuser.column.logMessage"/></th>
                        <th><spring:message code="logauthadminuser.column.adminId"/></th>
                        <th><spring:message code="logauthadminuser.column.menuId"/></th>
                    ...
                    <c:choose>
                        <c:when test="${logAuthAdminUsers != null and fn:length(logAuthAdminUsers) > 0}">
                            <c:forEach var="logAuthAdminUser" items="${logAuthAdminUsers}">
                                <tr>
                                    <td>${logAuthAdminUser.logType}</td>
                                    <td>
                                        <c:choose>
                                            <c:when test="${logAuthAdminUser.status == 'SUCCESS'}">
                                                <spring:message code="logauthadminuser.common.success"/>
                                            </c:when>
                                            <c:when test="${logAuthAdminUser.status == 'FAILURE'}">
                                                <spring:message code="logauthadminuser.common.failure"/>
                                            </c:when>
                                            <c:otherwise>
                                    ...
                                    <td>${logAuthAdminUser.logMessage}</td>
                                    <td>${logAuthAdminUser.adminUserId}</td>
                                    <td>${logAuthAdminUser.menuId}</td>
                                    <td>${logAuthAdminUser.ipAddress}</td>
                                    <td>
                                        <fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${logAuthAdminUser.logDatetime}" />
                                    </td>
                                </tr>
                            </c:forEach>
                        </c:when>
...
<script type="text/javascript">
    ...
    var form = $('#logAuthAdminUserForm');

    var urlConfig = {
        'listUrl':'${rootPath}/logauthadmin/list.html'
    };
