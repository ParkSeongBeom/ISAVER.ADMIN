 ...
 * @since 2014.05.20
 */
function OrganizationView(model) {
    var OrganizationView = new Object();
    OrganizationView._model = model;
...
//            if (String(data.orgSeq) == String(OrganizationView._model.getRootOrgId())) {
...
//            }
     ...
     * @date 2014.05.20
     */
    OrganizationView.setDetail = function (obj) {
...
//                console.error("[Error][MenuView.setDetail] " + e);
        ...
        OrganizationView.setSaveBefore();
     ...
     * @param obj
     */
    OrganizationView.setMenuTree = function (menuTreeModel) {
        var organizationCtrl = new OrganizationCtrl(OrganizationView._model);

        $(OrganizationView._model.getTreaArea()).dynatree({
            minExpandLevel: 2,
            debugLevel: 1,
            persist: true,
            ...
            ,onActivate: organizationCtrl.selectMenuTree
        });
     ...
     */
    OrganizationView.setOrgUserDetail = function(orgUserModel) {
     ...
     * 페이징 TAG 초기화 설정
     */
    OrganizationView.setPagingView = function() {
        $("#pageContainer").empty();
     ...
     */
    OrganizationView.setFindDetailBefore = function (data) {
        ...
        if (OrganizationView._model.getOrgId() == OrganizationView._model.getRootOrgId()) {
            ...
            $(formName + " [name='orgId']").val(OrganizationView._model.getOrgId());
     ...
     * 조직도 등록 전 초기화 모드
     */
    OrganizationView.setAddBefore = function() {
        $("[name='showHideTag']").hide();
        ...
        $(formName + " select[id=selectUpOrgSeq]").val(OrganizationView._model.getOrgId()).attr("selected", true);
        $("input:hidden[name='upOrgId']").val(OrganizationView._model.getOrgId());
     ...
     * 조직도 저장 전 초기화 모드
     */
    OrganizationView.setSaveBefore = function() {
     ...
     * 부서원 등록 팝업 창 띄우기
     */
    OrganizationView.openPopupUserPage = function() {
     ...
     * 부서 깊이 초기화
     */
    OrganizationView.resetOrgDepth = function() {
