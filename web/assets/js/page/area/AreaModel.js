...
function AreaModel() {
     ...
     * @returns {string|*}
     */
    AreaModel.setUpOrgId = function (upOrgId) {
     ...
     * @returns {string|*}
     */
    AreaModel.setOrgId = function (orgId) {
     ...
     * @param menuName
     */
    AreaModel.setOrgName = function (orgName) {
    ...
    AreaModel.processMenuTreeData = function (_list, _rootId) {
        ...
        function getParentNode(_children, item) {
            ...
            for (var i = 0, child; child = _children[i]; i++) {
                //부모를 찾았으면,
                if (String(child.id) === String(item.parentAreaId)) {
