...
import com.icent.jabber.repository.bean.TargetBean;
...
public class TargetSvcImpl implements TargetSvc {
    ...
    public ModelAndView findByTarget(Map<String, String> parameters) {
        TargetBean paramBean = AdminHelper.convertMapToBean(parameters, TargetBean.class);

        TargetBean targetBean = null;

        if(StringUtils.notNullCheck(paramBean.getTargetId())) {
            targetBean = targetDao.findByTarget(paramBean);
        }

        ModelAndView modelAndView = new ModelAndView();
        modelAndView.addObject("target",targetBean);
        return modelAndView;
    }

    @Override
    public ModelAndView saveTarget(Map<String, String> parameters) {
        TargetBean paramBean = AdminHelper.convertMapToBean(parameters, TargetBean.class, true, "yyyy-MM-dd HH:mm:ss");

        TransactionStatus transactionStatus = TransactionUtil.getMybatisTransactionStatus(transactionManager);
        try {

            targetDao.saveTarget(paramBean);
    ...
    public ModelAndView saveTargetFileSetting(Map<String, String> parameters) {
        ...
        try {
            ...
            targetDao.saveTargetFileSetting(paramBean);
            transactionManager.commit(transactionStatus);
        } catch(DataAccessException e){
            transactionManager.rollback(transactionStatus);
    ...
    public ModelAndView resetTarget() {
        ...
        try {
            for (int i=0; i<wasList.size(); i++){
                ...
                HttpPost wasHttpPost = new HttpPost(wasUrlBuilder.toString()+"/resetTarget.json");
        ...
        try {
            for (int i=0; i<apiList.size(); i++){
                ...
                HttpPost apiHttpPost = new HttpPost(apiUrlBuilder.toString()+"/management/resetTarget.json");
