...
public class AdminHelper {
     ...
     * @param totalCount
     */
    public static void setPageTotalCount(FindBean paramBean, Integer totalCount){
        paramBean.setTotalCount(totalCount);
        paramBean.setPageTotalCount((int) Math.ceil(totalCount / (double)paramBean.getPageRowNumber()));
    }
     ...
     * @return
     */
    public static AdminBean getAdminInfo(HttpServletRequest request){
        return (AdminBean)request.getSession().getAttribute(AdminResource.AUTHORIZATION_ADMIN);
    }
     ...
     * @param adminBean
     */
    public static void setAdminInfo(HttpServletRequest request, AdminBean adminBean){
        request.getSession().setAttribute(AdminResource.AUTHORIZATION_ADMIN, adminBean);
    }
    ...
    public static String getAdminIdFromSession(HttpServletRequest request){
        String adminId = null;
        try{
            AdminBean adminBean = getAdminInfo(request);
            adminId = adminBean.getAdminId();
        }catch(Exception e){
    ...
    /**
     * 국가코드 형식의 전화 포맷 변경
     * ex) +82 10 1234 4567 -> 01012344567
     */
    public static String convertPhoneFormat(String phoneNumber, boolean replaceFlag) {
        if(StringUtils.notNullCheck(convertNullToString(phoneNumber))){
            try {
                PhoneNumberUtil phoneUtil = PhoneNumberUtil.getInstance();
                Phonenumber.PhoneNumber number = phoneUtil.parseAndKeepRawInput(phoneNumber, "KR");
                phoneNumber = phoneUtil.format(number, PhoneNumberUtil.PhoneNumberFormat.NATIONAL);
                if(replaceFlag){
