(function (global, factory) {
  ...
  function addSvgToPdf(doc, svgElement, leftPosition, margin, callback) {
    ...
    var blob =new Blob(svgInfo.source, { type:'image/svg+xml;charset=utf-8' });
    var url = window.URL.createObjectURL(blob);
    ...
    image.onload =function() {
      canvas.width = svgInfo.width;
      canvas.height = svgInfo.height;
      context.drawImage(image,0,0);
      addPdf(doc, canvas, leftPosition, margin, callback);
    };
  }

  function addHtml2canvasPdf(doc, element, leftPosition, margin, callback){
    html2canvas(element).then(function (canvas) {
      addPdf(doc, canvas, leftPosition, margin, callback);
    });
  }

  function addPdf(doc, canvas, leftPosition, margin, callback){
      var imgData = canvas.toDataURL('image/png');
      var imgWidth = 295;
      var pageHeight = 210;
      var imgHeight = canvas.height * imgWidth / canvas.width;
      // 이전 이미지의 마지막 위치값
      var position = leftPosition%pageHeight+margin;
      // 현재 그려야될 위치값 (마진포함)
      var drawPosition = leftPosition+margin;
      // 이미지추가
      doc.addImage(imgData, 'PNG', 0, drawPosition, imgWidth, imgHeight);

      // 이미지 높이만큼 페이지 높이에 따라 추가 페이지 생성
      if(imgHeight+position>pageHeight){
        for(var i=1; i<Math.ceil((imgHeight+position)/pageHeight); i++){
          doc.addPage();
          drawPosition -= pageHeight;
          doc.addImage(imgData, 'PNG', 0, drawPosition, imgWidth, imgHeight);
        }
      }
      // 다음 이미지가 추가되어야할 포지션
      var nextPosition = imgHeight+drawPosition;
      // Callbaak
      if(callback!=null && typeof callback == 'function'){
        callback(doc,nextPosition);
      }
  }
  ...
  exports.save = save;
  exports.addSvgToPdf = addSvgToPdf;
  exports.addHtml2canvasPdf = addHtml2canvasPdf;
